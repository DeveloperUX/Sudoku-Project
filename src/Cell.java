/*
 * Moustafa Ismael
 * Mohammad Abdussalaam
 *Ayoub Benguedouar
 */


import java.awt.Color;
import java.awt.Dimension;
import java.awt.GridLayout;
import java.util.ArrayList;
import javax.swing.*;
import javax.swing.border.Border;

public class Cell extends JPanel {
    private boolean tf;
    int[] poss;
    private String sNum;
    public JPanel jPanel;
    private JLabel[] possibleSolutions;
    private JLabel actualSolution = null;
    private int[] nums = new int[9];
    private int cellRow;
    private int cellCol;
    
    public Cell() {
        
        jPanel = new JPanel();
        GridLayout layout = new GridLayout(3,3);        
        jPanel.setLayout(layout);
        Border raisedBevel = BorderFactory.createRaisedBevelBorder();
        jPanel.setBorder(raisedBevel);
        
        initialize();    
        Dimension d = new Dimension(50, 50);
        jPanel.setPreferredSize(d);
    }
    
    public Cell(int num){
        
        sNum = Integer.toString(num);
        jPanel = new JPanel();
        GridLayout layout = new GridLayout(1,1);        
        jPanel.setLayout(layout);
        Border raisedBevel = BorderFactory.createRaisedBevelBorder();
        jPanel.setBorder(raisedBevel);
        initSolved();
    }
    
     public void initSolved() {
            actualSolution = new JLabel();
            actualSolution.setText(sNum);
            actualSolution.setFont(new java.awt.Font("Times New Roman", 0, 35));    
            
            actualSolution.setVerticalAlignment(actualSolution.CENTER);
            actualSolution.setHorizontalAlignment(actualSolution.CENTER);
            jPanel.add(actualSolution);            
    }
     
     public void initialize() {
        possibleSolutions = new JLabel[9];
        Integer count = 1;
        JLabel counter;     
        
        for (int index=0; index<9; index++) {
            counter = new JLabel();
            counter.setVerticalAlignment(actualSolution.CENTER);
            counter.setHorizontalAlignment(actualSolution.CENTER);
            counter.setText(count.toString());
            possibleSolutions[index] = counter;
            jPanel.add(counter);
            nums[index] = count;
            count++;
        }
    }
     
    public boolean checkCell(int row, int col, SudokuNine sud){
        tf = false;
        String numPoss;
        if(!isSolution()){
            for(int i = 0; i<9; i++){
                if((!sud.isValid(row, col, nums[i]))&&nums[i]!=0){
                    possibleSolutions[i].setForeground(Color.RED);
                    nums[i] = 0;
                    tf = true;
                }
            }
            
        }
        return tf;
    }

    public void cleanCell(){
        if(!isSolution())
            for(int i = 0; i<9; i++)
                if(nums[i] == 0)
                    possibleSolutions[i].setText("");
    }

    public void onlyToActual(){
        int count = 0;
        int index = 0;
        for(int i = 0; i<9; i++){
            if(nums[i]==0)
                count++;
            else
                index = i;
        }
        if(count == 8){
            sNum = Integer.toString(nums[index]);

            jPanel.setLayout(new GridLayout(1,1));
            jPanel.removeAll();
            initSolved();            
        }
    }
    
    public int getActualNum(){
        int num = 0;
        if(isSolution())
            num = Integer.parseInt(sNum);
        return num;
    }
    
    public JPanel getPanel() {
        return jPanel;
    }
    
    public JLabel getLabel() {
        return actualSolution;
    }
    
    public int[] getPossiblesList(){
        int[] possibleNumList = new int[9];
        for(int i = 0; i<9; i++)
            possibleNumList[i] = nums[i];
        return possibleNumList;
    }
    public int getPosX(){
        return cellRow;
    }
    public int getPosY(){
        return cellCol;
    }
    
    public boolean isSolution() {
        if (actualSolution != null) {
            return true;
        }
        else {
            return false;
        }
    }
    
    
    
    ///////////heishman's 
       public boolean isPossibleInvalid(int labelNumber) {
      if(possibleSolutions[labelNumber].getForeground() == Color.RED)
         return true;
      else
         return false;
   }
   public int numOfPossibles() {
       return possibleSolutions.length;
   }

   public void setPossibleInvalid(int labelNumber) {
      possibleSolutions[labelNumber].setForeground(Color.RED);
   }
   ////////////// Ours Continued
   public void setPos(int row, int col){
       cellRow = row;
       cellCol = col;
   }
   public boolean compareTo(Cell cellToCompare) {
       boolean bool = false;
       int blockRow = getPosX()/3*3 + getPosY()/3;
       int toCompareBlock = cellToCompare.getPosX()/3*3 + cellToCompare.getPosY()/3;
       if (getPossiblesList() == cellToCompare.getPossiblesList())
           if(getPosX() == cellToCompare.getPosX()||
               getPosY() == cellToCompare.getPosY()||
               blockRow == toCompareBlock)
               bool = true;
       return bool;
   }            
   
   public boolean checkForPair(){
       int count = 0;
       tf = false;
        
        for(int i = 0; i<9; i++){
            if(nums[i]!=0) {
                count++;
            }
        }
       if(count == 2)
           tf = true;
       return tf;
   }
   
   public void changeCellColorToBlue() {
        
            for (int i=0; i<9; i++) {
                if (!possibleSolutions[i].equals(""))
                    possibleSolutions[i].setForeground(Color.BLUE);
        }      
   }
   
   
}

